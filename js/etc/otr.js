; (function () {
	...
	var onIncoming = function (nickname, msg, encrypted) {
		...
		if (mesg["type"] === "message") {
			Cryptodog.addToConversation(
				mesg["body"], nickname, Cryptodog.buddies[nickname].id, 'message'
			)
			if (Cryptodog.me.currentBuddy !== Cryptodog.buddies[nickname].id && !Cryptodog.buddies[nickname].ignored()) {
				Cryptodog.messagePreview(mesg["body"], nickname)
			}
		}

		if (mesg["type"] === "file") {
			Cryptodog.addToConversation(btoa(JSON.stringify({
					object: mesg, nick: nickname, win: Cryptodog.buddies[nickname].id })), nickname, Cryptodog.buddies[nickname].id, 'file')
		}
	}
	...
	var onOutgoing = function (nickname, message) {
		Cryptodog.socket.send({
			type: "chat",
			nickname: nickname,
			chatroom: Cryptodog.socket.currentServer.conference,
			object: {
				body: message
			}
		});
